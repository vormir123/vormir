...
import { serializeJSON } from "@vormir/util/security";
import { isFunction, isArray } from "@vormir/util/typecheck";

import Provider from "@vormir/component/provider";
...
const renderer = async (req, res, routerContext, customRenderers) => {
    ...
    const jsTags = jsTagGenerator(flushedChunks, assetsStats),
        ...
        reduxState =
            process.env.vormir_BUILD_ENV.redux !== false ? reduxStore.getState() : undefined,
        apolloState =
            process.env.vormir_BUILD_ENV.graphql !== false ? apolloClient.extract() : undefined;
