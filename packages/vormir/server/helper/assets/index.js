...
import type { HelperInterfaceType } from "@vormir/flow-typed/helper";
import fs from "fs-extra";
import { isArray } from "@vormir/util/typecheck";
            ...
            const getStatsInterval = setInterval(() => {
                ...
                if (fs.pathExistsSync(`${directory.STATS_DIR}/stats.json`)) {
                    ...
                    if (process.env.NODE_ENV === "development" && this.stats) {
                        ...
                        if (!process.env.vormir_BROWSER_OPEN) {
                            process.env.vormir_BROWSER_OPEN = true;
                            openBrowser(`${process.env.vormir_HOST}:${process.env.vormir_PORT}`);
                        }
                    }
